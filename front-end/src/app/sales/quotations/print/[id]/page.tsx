...
import { fetchQuotationDetails, fetchCompany, contentUrl, type CompanyRow } from "@/lib/api";
import { Button } from "@/components/ui/button";
import { Loader2, Printer, X, Phone, Mail, MapPin, Calculator, Calendar } from "lucide-react";
...
function PrintContent() {
  ...
  useEffect(() => {
    if (autoPrint && !loading && quotation && !printedRef.current) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-500 font-medium tracking-tight">Preparing Quotation...</p>
        </div>
      </div>
    );
  }
  ...
  if (!quotation) {
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center space-y-4">
          <p className="text-rose-500 font-bold">Error: Quotation Not Found</p>
      ...
      <div className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200 print:hidden px-6 py-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-400 text-xs font-mono ml-2">{quotation.quotation_no}</span>
          </div>
        ...
        <div className="mx-auto w-[210mm] min-h-[297mm] bg-white shadow-2xl print:shadow-none print:w-full print:border-none px-12 py-10 text-slate-900 border border-slate-200">
          ...
          <div className="flex justify-between items-start border-b border-slate-200 pb-4">
            <div className="space-y-1">
              ...
              <div>
                <h1 className="text-2xl font-black tracking-tighter text-slate-900 uppercase">Sales Quotation</h1>
                <p className="text-slate-400 font-mono text-[10px] tracking-widest leading-none">NO: {quotation.quotation_no}</p>
              </div>
            </div>
            ...
            <div className="text-right space-y-0.5 max-w-xs pt-1">
              ...
              <div className="text-[10px] text-slate-500 whitespace-pre-line leading-tight">
                {quotation.location_address || company?.address || "Main Street, Colombo"}
                <div className="mt-1 space-y-0">
                  <div className="flex items-center justify-end gap-1.5 font-medium text-slate-700 italic underline underline-offset-1 decoration-slate-100">
                    <Phone className="w-2 h-2"/> {quotation.location_phone || company?.phone}
                  </div>
                  {(quotation.location_tax_no || company?.tax_no) && (
                    <div className="flex items-center justify-end gap-1.5 font-bold text-slate-900 uppercase text-[9px] mt-0.5">
                      {company?.tax_label || "TAX ID"}: {quotation.location_tax_no || company?.tax_no}
                    </div>
                  )}
          ...
          <div className="grid grid-cols-2 gap-6 py-4">
            <div className="space-y-4">
              <div className="space-y-1">
                <h4 className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 mb-0.5">Quotation For</h4>
                <p className="text-sm font-black text-slate-900 leading-tight">{quotation.customer_name}</p>
                <div className="text-[11px] text-slate-600 space-y-0 mt-0.5">
                  <p className="font-semibold text-slate-800">{quotation.customer_phone}</p>
                  {quotation.customer_address && <p className="whitespace-pre-line leading-snug">{quotation.customer_address}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[11px] py-0.5 border-b border-slate-50">
                ...
                <span className="font-bold text-slate-700">{new Date(quotation.issue_date).toLocaleDateString('en-GB')}</span>
              </div>
              <div className="flex justify-between text-[11px] py-0.5 border-b border-slate-50">
                <span className="text-slate-400 font-medium">Valid Until</span>
                <span className="font-bold text-slate-700">{quotation.expiry_date ? new Date(quotation.expiry_date).toLocaleDateString('en-GB') : "N/A"}</span>
              </div>
              <div className="mt-2">
                <div className={`text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded inline-block border leading-none bg-slate-50 text-slate-600 border-slate-200`}>
                  {quotation.status}
                </div>
              </div>
            </div>
            ...
            <div className="w-72 space-y-3">
              <div className="space-y-1 pb-3 border-b border-slate-100 text-xs">
                <div className="flex justify-between items-center text-slate-600">
                  ...
                  <span className="font-bold tabular-nums">LKR {Number(quotation.subtotal).toFixed(2)}</span>
                </div>

                {quotation.taxes && quotation.taxes.length > 0 ? (
                  ...
                  Number(quotation.tax_total) > 0 && (
                    <div className="flex justify-between items-center text-slate-600">
                      ...
                      <span className="font-bold tabular-nums">LKR {Number(quotation.tax_total).toFixed(2)}</span>
                    </div>
                  )
                )}
              </div>
              ...
              <div className="flex justify-between items-center py-1">
                ...
                <span className="font-black text-slate-900 text-2xl tracking-tighter tabular-nums">{formatCurrency(quotation.grand_total)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 space-y-6">
            {quotation.notes && (
              <div className="space-y-1">
                <h5 className="text-[9px] font-black uppercase tracking-widest text-slate-400">Terms & Conditions</h5>
            ...
            <div className="flex flex-col items-center justify-center pt-8 text-center space-y-2 border-t border-slate-50 opacity-40">
              ...
              <p className="text-[8px] text-slate-400 font-medium px-32 leading-tight uppercase tracking-widest">
                This is a computer generated estimate. Prices are valid until the expiry date mentioned above.
              </p>
            </div>
          </div>
