...
import { fetchCompany, fetchLocations, fetchPurchaseOrder, fetchSupplier, type CompanyRow, type ServiceLocationRow, type TaxRow } from "@/lib/api";
...
export default function PurchaseOrderPrintPage() {
  ...
  useEffect(() => {
    const run = async () => {
      setLoading(true);
      try {
        const locIdRaw = typeof window !== "undefined" ? window.localStorage.getItem("location_id") : null;
        const locId = locIdRaw ? Number(locIdRaw) : null;

        const [data, comp, locs] = await Promise.all([fetchPurchaseOrder(String(id)), fetchCompany(), fetchLocations()]);
        setPo((data as any)?.purchase_order ?? null);
        setItems(Array.isArray((data as any)?.items) ? (data as any).items : []);
        setCompany((comp as any) ?? null);

        const poRow: any = (data as any)?.purchase_order ?? null;
        const sid = Number(poRow?.supplier_id ?? 0);
        if (sid > 0) {
          try {
    ...
    <>
      ...
      <div className="print:hidden sticky top-0 z-10 bg-background/90 backdrop-blur border-b">
        <div className="mx-auto max-w-4xl px-4 py-3 flex items-center justify-between gap-3">
          <div className="font-semibold">Print Purchase Order</div>
      ...
      <div className="bg-muted/20 min-h-screen py-8 print:bg-white print:py-0">
        <div className="mx-auto max-w-4xl px-4">
          {loading ? (
            ...
            <div className="py-24 text-center text-muted-foreground">Purchase order not found.</div>
          ) : (
            <div className="bg-white border shadow-sm rounded-md p-6 print:border-0 print:shadow-none print:rounded-none">
              <div className="flex items-start justify-between gap-6">
                ...
                <div className="text-sm text-right">
                  <div className="text-xl font-bold leading-tight">Purchase Order</div>
                  <div className="text-sm text-muted-foreground mt-1">
                    PO No: <span className="font-semibold text-foreground">{po.po_number ?? `#${id}`}</span>
                  </div>
                  <div className="my-3 border-t" />
                  <div>
                    <span className="text-muted-foreground">Supplier: </span>
                    <span className="font-semibold">{po.supplier_name ?? po.supplier_id ?? "-"}</span>
                  </div>
                  {supplierTaxRegNo ? (
                    <div>
                  ...
                  <div>
                    ...
                    <span>{po.ordered_at ?? "-"}</span>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Expected Date: </span>
                    <span>{po.expected_at ?? "-"}</span>
                  </div>
                  <div>
                  ...
                  {po.last_grn_number ? (
                    <div>
                      ...
                      <span className="font-semibold">{po.last_grn_number}</span>
                    </div>
                  ) : null}
                </div>
              </div>
              ...
              {po.notes ? (
                <div className="mt-4 text-sm">
                  ...
                  <div className="whitespace-pre-wrap">{String(po.notes)}</div>
                </div>
              ) : null}
              ...
              <div className="mt-8 grid grid-cols-3 gap-8 text-sm">
                {["Checked By", "Certified By", "Authorized By"].map((label) => (
