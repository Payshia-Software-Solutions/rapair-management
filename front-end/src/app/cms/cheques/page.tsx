...
import { fetchChequeInventory, updateChequeStatus } from "@/lib/api";
...
import { Search, CheckCircle2, XCircle, AlertCircle, Clock, Loader2 } from "lucide-react";
      ...
      <div className="p-6 space-y-6 w-full">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {Object.entries(STATUS_CONFIG).map(([status, cfg]) => {
            ...
            const total = cheques.filter(c => c.status === status).reduce((s, c) => s + Number(c.amount || 0), 0);
            return (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map((c, idx) => {
                    ...
                    return (
                      <tr key={c.id} className={`border-b border-border/50 hover:bg-muted/20 transition-colors ${idx % 2 === 0 ? '' : 'bg-muted/10'}`}>
                        ...
                        <td className="px-4 py-3 text-right">
                          {c.status === 'Pending' && (
                            <Button size="sm" variant="outline" className="text-xs"
                              onClick={() => { setSelected(c); setNewStatus('Cleared'); setUpdateOpen(true); }}>
                              Update Status
                            </Button>
                          )}
                        </td>
