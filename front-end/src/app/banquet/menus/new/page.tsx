...
export default function CreateBanquetMenuPage() {
  ...
  const loadParts = async () => {
    ...
    try {
      const res = await api("/api/part/list");
  ...
  const addTempItem = () => {
    ...
    const items = [...formData.items, { ...newItem, id: Date.now() }];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.name || !formData.price_per_pax) {
    ...
    <DashboardLayout>
      <div className="flex flex-col gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-bold tracking-tight">Create New Menu</h2>
            <p className="text-muted-foreground">Define a new preset menu and its initial dish list.</p>
          </div>
        </div>
                ...
                <div className="space-y-4">
                  ...
                  {formData.items.length === 0 && (
                    <div className="text-center py-12 bg-muted/5 rounded-3xl border border-dashed">
                      ...
                      <p className="text-sm text-muted-foreground italic">Your menu is currently empty. Add some dishes above.</p>
                    </div>
                  )}
                </div>
              </CardContent>
          ...
          <div className="space-y-6">
            <Card className="bg-primary text-white border-none shadow-xl sticky top-6">
              ...
              <CardContent className="space-y-6">
                <div className="space-y-1">
                   <p className="text-xs text-white/60">Selected Title</p>
                   <p className="text-xl font-bold">{formData.name || "Untitiled Menu"}</p>
                </div>
                ...
                <div className="space-y-2 pt-4 border-t border-white/10">
                   ...
                   <div className="flex justify-between text-sm">
                      ...
                      <span className="font-bold">{formData.is_active ? "Active" : "Draft"}</span>
                   </div>
                </div>
              </CardContent>
            ...
            <div className="p-6 rounded-3xl bg-amber-50 border border-amber-100 space-y-3">
               ...
               <p className="text-xs text-amber-700 leading-relaxed">
                  Try to categorize items clearly. A balanced menu typically includes 1-2 Appetizers, a Main Course, and 1-2 Dessert options.
               </p>
            </div>
          </div>
        </div>
