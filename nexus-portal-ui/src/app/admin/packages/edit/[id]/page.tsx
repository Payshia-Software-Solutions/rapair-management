...
import { 
  ...
  ShoppingCart, Users, Globe, Sparkles, Store, Smartphone, Wallet, Factory, Building2, Music, BarChart, LayoutDashboard, Truck, Zap, Shield, Check, Layers, Plus
} from 'lucide-react';
...
export default function PackageEditPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setSuccess('Package tier updated successfully');
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-black tracking-tight text-[#09090b] dark:text-white">
                {packageData.name || 'Edit Package Tier'}
              </h1>
            ...
            <p className="text-xs text-[#71717a] dark:text-[#a1a1aa] mt-0.5">
              Configure tier parameters, system modules, SLAs and pricing.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5">
          ...
          >
            ...
            {saving ? 'Saving Tier…' : 'Save Changes'}
          </button>
        </div>
      </div>
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      ...
                      setPackageData({
                        ...packageData, 
                        monthly_price: m,
                        yearly_price: m && !packageData.yearly_price ? (parseFloat(m) * 12 * 0.80).toFixed(2) : packageData.yearly_price
                      });
                    }}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      const m = parseFloat(packageData.monthly_price || '0');
                      setPackageData({...packageData, yearly_price: (m * 12 * 0.80).toFixed(2)});
                    }}
          ...
          <div className="rounded-2xl border border-[#e4e4e7] bg-white p-6 dark:border-[#27272a] dark:bg-[#18181b] shadow-sm space-y-5">
            <div>
              ...
              <h3 className="text-lg font-black text-[#09090b] dark:text-white mt-1 truncate">
                {packageData.name || 'Untitled Tier'}
              </h3>
            ...
            <div className="pt-2">
              ...
              >
                ...
                {saving ? 'Updating Package…' : 'Save Package Tier'}
              </button>
            </div>
          </div>
